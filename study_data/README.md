...
2. **Hold some prompts out** (`reserved_ids.json`). Those were used to build the coding guidebook. They must not appear in this study set.
...
8. **Freeze 100** into `prompts_eval_v1.csv` (ids `E001`–`E100`). Random seed for the final draw: `20262934`.  
   `prompts_eval_v1.meta.json` stores extra info (e.g. original DevGPT link). The coding tool only needs the CSV.
